...
var si SCROLLINFO
var iDeltaPerLine, iAccumDelta int32
...
func WndProc(hwnd HWND, msg uint32, wParam, lParam uintptr) (result uintptr) {
    ...
    case WM_MOUSEWHEEL:
        ...
        iAccumDelta += int32(int16(int32(wParam) >> 16 & 0xffff)) // 120 or -120

        for ; iAccumDelta >= iDeltaPerLine; iAccumDelta -= iDeltaPerLine {
    ...
    case WM_SETTINGCHANGE:
        var ulScrollLines uint64
        SystemParametersInfo(SPI_GETWHEELSCROLLLINES, 0, unsafe.Pointer(&ulScrollLines), 0)

        // ulScrollLines usually equals 3 or 0 (for no scrolling)
        // WHEEL_DELTA equals 120, so iDeltaPerLine will be 40

        if ulScrollLines > 0 {
            iDeltaPerLine = int32(WHEEL_DELTA / ulScrollLines)
        } else {
            iDeltaPerLine = 0
        }
